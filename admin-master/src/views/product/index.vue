  ...
  >
    ...
    <template #operate="scope">
      <el-button
        size="mini"
        type="primary"
        @click="$router.push(`/product/productdetail/${scope.row.id}`)"
      >
        编辑
      </el-button>
      <el-button size="mini" type="danger" @click="deldata($event)">
        删除
      </el-button>
    </template>
    <template #img="scope">
      <div class="img">
        <img
          ...
          :src="`http://localhost:3001/public/pro-img/${scope.row.img}`"
        />
      </div>
    </template>
  </pro-table>
